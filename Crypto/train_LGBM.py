...
raw_train = pd.read_csv('Crypto/train_with_F.csv', index_col='datetime')
raw_test = pd.read_csv('Crypto/test_with_F.csv',index_col='datetime')

# 2.切分数据输入：特征 输出：预测目标变量


# 3.切分训练集、测试集,切分比例7.5 : 2.5
train_X = raw_train.drop(['Target'], axis=1)
test_X = raw_test.drop(['Target'], axis=1)

train_y = raw_train['Target']
test_y = raw_test['Target']
...
my_model = lgb.train(params, lgb_train, num_boost_round=20, valid_sets=lgb_eval, early_stopping_rounds=20)
