...
X_train = pta.wcp(X_train_raw['High'],X_train_raw['Low'],X_train_raw['Close'])

# Merge X and y, clean NA values
train = pd.concat([X_train, y_train], ignore_index=False, join='inner',axis=1)
print(train)

# train = train.dropna(subset = ['WCP'])

# Apply the same to test sub dataset
X_test = pta.wcp(X_test_raw['High'],X_test_raw['Low'],X_test_raw['Close'])
...
model.fit(train[['WCP']], train['Predict_1min'])

# Calculate prediction from model
predictions = model.predict(test[['WCP']])
...
plt.scatter(test['WCP'], test['Predict_1min'], color='black')

plt.plot(test['WCP'], predictions, color='blue', linewidth=3)
plt.xticks(())
plt.yticks(())
plt.show()